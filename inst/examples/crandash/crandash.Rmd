...
output$download_rate <- renderValueBox({
...
```{r}
valueBoxOutput("total_downloads")
```
...
```{r}
bubblesOutput("popularity")
```


```{r, context="server"}
output$popularity <- renderBubbles({
  if (nrow(pkgData()) == 0)
    return()
...
```{r}
tableOutput("percent_downloads")
```

```{r, context="server"}
output$percent_downloads <- renderTable({
  pkgData() %>%
    group_by(package) %>%
    tally() %>%
    arrange(desc(n), tolower(package)) %>%
...
```{r}
tableOutput("recent_downloads")
```


```{r, context="server"}
output$recent_downloads <- renderTable({
