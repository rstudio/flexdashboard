...
### Downloads per sec (last 5 min) {.value-box data-icon="fa-area-chart"}
...
renderValueBox({
  downloadRate <- formatC(downloadRate(), digits = 1, format = "f")
  valueBox(
    downloadRate,
    color = if (downloadRate >= input$rateThreshold) "warning" else "primary"
  )
})
