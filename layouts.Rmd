...
This behavior is controlled via the `vertical_layout` output option, which defaults to "fill". Filling the page is generally a good choice when you have only one or two charts vertically stacked, whereas scrolling is generally a better choice for three or more charts vertically stacked. The sample layouts below reflect these general guidelines.

## Layouts

### Chart Stack (Fill)

This layout is a simple stack of two charts. Since there are only two charts the need to scroll is unlikely, so `vertical_layout: fill` is specified so they grow to fill the page vertically. Note that one chart or the other could be made vertically taller by specifying the `data-height` attribute.
...
This layout fills the page completely and gives prominence to a single chart at the top (with two secondary charts included below). To achieve this layout it uses `vertical_layout: fill`, `orientation: rows` and specifies `data-height` attributes on each row to establish their relative sizes.
...
This layout fills the page completely and gives prominence to a single chart on the left (with two secondary charts included to the right). To achieve this layout it uses `vertical_layout: fill` and specifies and specifies `data-width` attributes on each column to establish their relative sizes.
...
This layout is a 2x2 grid of charts that uses `vertical_layout: fill`. Note that depending on the ideal display size for the charts it might be better to allow the page to scroll (`vertical_layout: scroll`). Note also that `orientation: rows` is used to ensure that the baselines of charts line up horizontally.
