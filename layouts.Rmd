...
This layout gives prominence to a single chart at the top, with two secondary charts included below. To achieve this layout it uses `orientation: rows`, specifies `fill_page: true`, and specifies `data-height` attributes on each row to establish their relative sizes.
...
This layout gives prominence to a single chart on the left, with two secondary charts included to the right. To achieve this layout it uses `fill_page: true` and specifies and specifies `data-width` attributes on each column to establish their relative sizes.
