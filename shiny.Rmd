...
There's one other important detail to understand related to the total height occupied by embedded Shiny components. By default, flexdashboard uses a vertical layout that fills the page (e.g. `vertical_layout: fill`). In this scenario, the Shiny UI defined above will have it's size automatically determined by the size of it's flexdashboard container.

However, even when using a fill layout components in flexdashboards also need to provide a fixed height layout. This is because dashboards will always scroll rather than fill the screen on mobile phones. Embedded shiny components that use `fillCol` or `fillRow` will be automatically assigned a scrolling height of 500 pixels, however the ideal height for your situation may be different. In the above example you can see that a non-default height of 600 pixels is specified via `fillCol(height = 600, ...)`.
...
Note that when embedding Shiny components using an inline application definition an `<iframe>` is created to host the application. In contrast, when using [Shiny Modules] the components are included inline on the page, which makes them generally preferable to inline applications.
