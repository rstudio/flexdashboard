...
Charts will always be automatically scaled to fit within their allocated flexbox region. Charts based on [htmlwidgets](http://www.htmlwidgets.org) can dynamically re-size themselves, so their scaling will nearly always result in charts that perfectly fit the bounds of their containers.
...
3. A non-default background color is specified for the third value box via the inclusion of the `.bg-warning` attribute. Available background color attributes include `.bg-primary` (the default), `.bg-info`, `.bg-success`, `.bg-warning`, and `.bg-danger`. You can also specify an arbitrary CSS background color using e.g. `data-background=rgb(100,100,100)`.

4. The R code chunk within the section prints a single value (in the example these are hard-coded literals, but would more typically be the result of some computations).
