...
You can use several types of charts with flexdashboard, each of which has its advantages and disadvantages:

1. Charts based on [htmlwidgets](http://www.htmlwidgets.org). These charts are interactive (using JavaScript) and standalone (they don't require a connection to a server running R, rather they are deployed as part of a static web page). One disadvantage of htmlwidgets is that they embed their data directly in their host web page, so may encounter performance problems for larger datasets.

2. Charts based on standard R graphics (e.g. base, lattice, grid). These charts are static (they are just PNG images) and standalone (no server required). One disadvantage of these charts is that they take a bit more work to scale correctly into dashboard panes (see [Chart Scaling] for further details).

3. Charts based on [Shiny](shiny.html). These charts can created using either of the two techniques described above, however gain additional dynamic and interactive capabilities via Shiny. Note however that dashboards which use Shiny are no longer static web pages, and require [deployment](http://shiny.rstudio.com/deploy/) to a Shiny Server).
...
#### Scaling for htmlwidgets and shiny

Charts based on [htmlwidgets](http://www.htmlwidgets.org) can dynamically re-size themselves, so their scaling will nearly always result in charts that perfectly fit the bounds of their containers.

It's also possible to use charts created using standard R graphics (base, lattice, grid, etc.) with flexdashboard. Within dynamic (Shiny) dashboards these charts are also automatically sized to fit within their containers so long as they are wrapped within a call to [renderPlot](http://shiny.rstudio.com/reference/shiny/latest/renderPlot.html).

#### Scaling for base, lattice, and grid
...
A wide selection of icons are available, see the [Icon Sets] section below for details.
...
You can use CSS to customize the colors used within the navigation bar. If you do this, realize that the CSS for navigation bars defines color for many elements and states, so you should be careful to override all of the required elements. Note also that you can change the entire theme of the dashboard easily using the `theme` option (see [Dashboard Theme] below for details).

The navigation bar for flexdashboard uses the `navbar-inverse` class for each of its themes. Here is the `navabar-inverse` CSS for the default "cosmo" theme used by flexdashbaord:
...
2. The `data-icon` attribute can be optionally specified to include an icon within the value box (see [Icon Sets] for details on available icons).

3. The R code chunk within the section calls the `valueBox` function to emit a value.

4. The third code chunk makes the background color of the value box dynamic using the `color` parameter. Available colors include "primary", "info", "success", "warning", and "danger" (the default is "primary"). For custom colors you can also specify any valid CSS color (e.g. "#ffffff", "rgb(100,100,100)", etc.).
