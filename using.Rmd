...
There are also several ways to add interactivity to flexdashboards. You can use [htmlwidgets] to add client-side JavaScript interactivity (data tips, panning, zooming, etc.) and [Shiny](shiny.html) to build fully custom interactions in R.
...
<img src="images/mobile-phone.png" width=187 height=332 class="screenshot" align="right" style="margin-left: 15px; margin-bottom: 15px;"/>
...
1) By default R graphics are rendered twice, once at their natural `fig.width` and `fig.height`, and once at a mobile-optmized size (3.75 x 4.8 inches). When displaying on a mobile phone in portait orientation the mobile size is used (this can be controlled via the `fig_mobile` option).

2) Simple tables always show all of their rows (this is so that scrolling the table doesn't interfere with scrolling the mobile display).

3) DataTables always use pagination (again, so that scrolling the DataTable doesn't interfere with scrolling the overall display).
